...
def load_data():
    data = {
        ...
        'FNN': pd.read_csv("forecast_fnn.csv").merge(
            prophet_data[['ds', 'y']], on='ds', how='left'
        ),
        'FCNN': pd.read_csv("forecast_fcnn.csv").merge(
            prophet_data[['ds', 'y']], on='ds', how='left'
        ),
        'XGBoost': pd.read_csv("forecast_xgb.csv").merge(
            prophet_data[['ds', 'y']], on='ds', how='left'
        ),
        'Ансамбль': pd.read_csv("forecast_ensemble.csv").merge(
            prophet_data[['ds', 'y']], on='ds', how='left'
        )
    }
    for key in data:
